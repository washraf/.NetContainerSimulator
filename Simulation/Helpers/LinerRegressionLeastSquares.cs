...
{
    /// <summary>
    /// Linear Regression Least squares calculator from 
    /// https://msdn.microsoft.com/en-us/magazine/mt238410.aspx
    /// </summary>
    public class LinerRegressionLeastSquares
    {
        private readonly double[][] _data;
        private double[] _coeffecients;
        private double _r;

        public LinerRegressionLeastSquares(double[][] data)
        {
            _data = data;
            Solve();
            RSquared();
        ...
        public void Solve()
        {
            ...
            _coeffecients = result;
            //return result;
        } // Solve

        public double FindEstimatedValue(params double[] x)
        {
            // x1 = education, x2 = work, x3 = sex
            double result = _coeffecients[0]; // the constant
            for (int i = 1; i < _coeffecients.Length; i++)
            {
                result += (x[i - 1]*_coeffecients[i]);
            }
            return result;
        }

        private void RSquared()
        {
            ...
            double ySum = 0.0;
            for (int i = 0; i < rows; ++i)
                ySum += _data[i][cols - 1]; // last column
            ...
            for (int i = 0; i < rows; ++i)
            {
                y = _data[i][cols - 1]; // get actual y

                predictedY = _coeffecients[0]; // start w/ intercept constant
                for (int j = 0; j < cols - 1; ++j) // j is col of data
                    predictedY += _coeffecients[j + 1]*_data[i][j]; // careful
            ...
            {
                _r = 1.0 - (ssr/sst);
                Console.WriteLine($"Rquared is {_r}");
            }
        }
